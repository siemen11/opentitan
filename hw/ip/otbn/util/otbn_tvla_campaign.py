...
class TVLASim(StandaloneSim):
    def __init__(self, trace_hw_file: Optional[TextIO] = None):
        super().__init__()
        self.trace_hw_file = trace_hw_file
        if self.trace_hw_file is not None:
            self._tvla_init()

    def _on_retire(self, verbose, insn):
        if self.trace_hw_file is not None:
...
class TVLAAccumulator:
    ...
    def add_trace_hws(self, pc, occ, set_idx, hws):
    ...
    def compute_t_test(self, pc, occ, model_idx):
...
def generate_reference_trace(simulator_path, elf_path, dwarf_map, cfg):

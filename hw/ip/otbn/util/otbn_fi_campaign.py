...
class FISim(StandaloneSim):
    def __init__(self, fi_skip_pc: Optional[int] = None, fi_skip_occurrence: int = 0):
        super().__init__()
...
def get_symbol_offsets(elf_path, target_symbols):
...
def parse_dmem(dmem_dump_path, offsets, sizes):
...
def build_pc_to_line_map(elf_path):
...
def parse_trace_for_pcs(trace_file):
    pc_list = []
...
def annotate_trace_file(trace_path, dwarf_map):
...
def generate_boolean_shares(secret, share_size, modulus=None):
    r = random.getrandbits(share_size * 8)
    return secret ^ r, r


def generate_additive_shares(secret, share_size, modulus=None):
    if modulus is None:
...
def generate_scalar_blind_shares(secret, share_size, modulus=None):
    if modulus is None:
...
def generate_shares(mode, secret, share_size, modulus=None):
    local_generators = {
        "boolean": generate_boolean_shares,
        "additive": generate_additive_shares,
        "scalar_blind": generate_scalar_blind_shares,
...
def create_dmem_override(secrets_cfg, out_path):
    trace_dict = {}
...
def unmask_value(regs, col_sym, mode, size, modulus_str=None):
    syms = col_sym.split(",")
...
def main():
    ...
    def parse_secret_args(arg_list):
        secrets = []

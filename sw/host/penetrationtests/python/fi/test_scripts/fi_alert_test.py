...
from test.penetrationtests.fi.host_scripts import fi_alert_functions
from target.communication.fi_alert_commands import OTFIAlert
...
class AlertFiTest(unittest.TestCase):
    ...
    def test_char_alert_trigger(self):
        ...
        for alert in range(63):
            ...
            if translated_alert not in exception_list:
                actual_result, got_response = fi_alert_functions.char_alert_trigger(target, alert)
                alert_class = \
                    common_library.default_alert_config["alert_classes"][translated_alert]
                alert_enabled = \
                    common_library.default_alert_config["enable_alerts"][translated_alert]
                enabled_class = \
                    common_library.default_alert_config["enable_classes"][alert_class]
                threshold = \
                    common_library.default_alert_config["accumulation_thresholds"][alert_class]
                reset_enabled = (3 in common_library.default_alert_config["signals"])

                if reset_enabled and alert_enabled and enabled_class and (threshold == 0):
                    self.assertEqual(got_response, False,
                                     msg=f'Expected a reset but got an output. \
                                        Failed for input {alert} and response {actual_result}')
                else:
                    self.assertEqual(got_response, True,
    ...
    def test_char_alert_sensor_ctrl_trigger(self):
        actual_result, got_response = fi_alert_functions.char_alert_sensor_ctrl_trigger(target)
        sensor_cntr_recov = case_to_alert_list_map[50]
        sensor_cntr_fatal = case_to_alert_list_map[51]
        alert_recov_enabled = \
            common_library.default_alert_config["enable_alerts"][sensor_cntr_recov]
        alert_fatal_enabled = \
            common_library.default_alert_config["enable_alerts"][sensor_cntr_fatal]
        sensor_ctrl_en_fatal = common_library.default_sensor_config["sensor_ctrl_en_fatal"][0]
        reset_enabled = (3 in common_library.default_alert_config["signals"])

        if reset_enabled and ((sensor_ctrl_en_fatal and alert_fatal_enabled) or
    ...
    def test_char_alert_ibex_sw_fatal(self):
        actual_result, got_response = fi_alert_functions.char_alert_ibex_sw_fatal(target)
        ibex_sw_fatal = case_to_alert_list_map[44]
        alert_class = common_library.default_alert_config["alert_classes"][ibex_sw_fatal]
        alert_fatal_enabled = common_library.default_alert_config["enable_alerts"][ibex_sw_fatal]
        reset_enabled = (3 in common_library.default_alert_config["signals"])
        enabled_class = common_library.default_alert_config["enable_classes"][alert_class]
...
if __name__ == "__main__":
    ...
    target_cfg = TargetConfig(target_type, firmware_path, opentitantool=opentitantool_path)

...
from sw.host.penetrationtests.python.util.chip import Chip
from sw.host.penetrationtests.python.util.cw_fpga import CWFPGA
...
class Target:
    ...
    def __init__(self, target_cfg: TargetConfig) -> None:
        ...
        self.com_interface = self._init_communication(self.find_target_port(), self.baudrate)
    ...
    def _init_target(self):
        ...
        target = None
        if (
            self.target_cfg.target_type == "cw305"
            or self.target_cfg.target_type == "cw310"
            or self.target_cfg.target_type == "cw340"
        ):
            target = CWFPGA(
    ...
    def _init_communication(self, port, baudrate):
        ...
        # TODO This is set because on the chip it never mattered.
        com_interface.timeout = 1
        return com_interface
    
    # TODO Fix that this works also for the FPGA and have a more specific description for the chip.
    ...
    def print_all(self, max_tries=20):
        it = 0
        while it != max_tries:
    ...
    def dump_all(self, max_tries=20):
        it = 0
        while it != max_tries:

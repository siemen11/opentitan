...
const MLDSA_87_PK_BYTES: usize = 2592;
const MLDSA_87_SIG_BYTES: usize = 4627;
const MLDSA_87_SEED_BYTES: usize = 32;
const MLDSA_87_CTX_MAX_BYTES: usize = 255;
...
fn run_mldsa_testcase(
    ...
    match test_case.operation.as_str() {
        "sign" => {
            if test_case.private_seed.len() != MLDSA_87_SEED_BYTES
                || test_case.context.len() > MLDSA_87_CTX_MAX_BYTES
                || !test_case.rnd.is_empty()
            {
                if test_case.result {
                    log::info!(
                        "PASSED sign test #{} (external rnd expected, skipping sign test)",
                        test_case.test_case_id
                    );
                } else {
                    log::info!(
                        "PASSED sign test #{} (invalid input length or external rnd expected)",
                        test_case.test_case_id
                    );
                }
                return Ok(());
            ...
            let sig_matches = sig_res.signature[..sig_res.signature_len] == test_case.signature[..];

            if test_case.result != sig_matches {
                if KNOWN_SIGN_FAILURES.contains(&test_case.test_case_id) {
                    log::info!(
                        "KNOWN FAILURE sign test #{}: expected = {}, actual = {}",
                        test_case.test_case_id,
                        test_case.result,
                        sig_matches
                    );
                    ...
                    log::info!(
                        "FAILED sign test #{}: expected = {}, actual = {}",
                        test_case.test_case_id,
                        test_case.result,
                        sig_matches
                    );
        ...
        "verify" => {
            if test_case.public_key.len() != MLDSA_87_PK_BYTES
                || test_case.signature.len() != MLDSA_87_SIG_BYTES
                || test_case.context.len() > MLDSA_87_CTX_MAX_BYTES
            {
                if test_case.result {
                    log::info!(
                        "FAILED verify test #{}: expected valid, but input parameter lengths invalid",
                        test_case.test_case_id
                    );
                    *fail_counter += 1;
                } else {
                    log::info!(
                        "PASSED verify test #{} (invalid input length expected)",
                        test_case.test_case_id
                    );
                }
                return Ok(());

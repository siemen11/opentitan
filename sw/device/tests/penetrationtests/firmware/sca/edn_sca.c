 ...
 * transmitted from the 128-bit FIFO within the EDN to the Ibex RND_DATA register.
 * 
 * To do so, this function first clears the 128-bit FIFO by reading it. When the
 * FIFO is empty, the EDN sends a request to the CSRNG. When the FIFO again is full,
 * set the SCA trigger and read the randomness from the FIFO to the RND_DATA
 * register. 
 *

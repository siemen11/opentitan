...
static status_t handle_mldsa_sign(ujson_t *uj) {
  ...
  if (uj_seed.private_seed_len != 32) {
    LOG_ERROR("Incorrect private seed length: expected = 32, got = %d",
              uj_seed.private_seed_len);
    return INVALID_ARGUMENT();
  }
  if (uj_ctx.context_len > kOtcryptoMldsa87CtxMaxBytes) {
    LOG_ERROR("Context too long: %d", uj_ctx.context_len);
    return INVALID_ARGUMENT();
  }
...
static status_t handle_mldsa_verify(ujson_t *uj) {
  ...
  if (uj_pk.public_key_len != kOtcryptoMldsa87PkBytes) {
    LOG_ERROR("Incorrect public key length: expected = %d, got = %d",
              kOtcryptoMldsa87PkBytes, uj_pk.public_key_len);
  ...
  otcrypto_const_word32_buf_t sig =
      otcrypto_make_const_word32_buf(sig_data, kOtcryptoMldsa87SigWords);
